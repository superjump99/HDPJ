...
if __name__ == '__main__':
    ...
    for folder_name in os.listdir(f"{base_path}/{working_done_path}/{space}/{dataset}"):
        ...
        if not os.path.exists(f"{base_path}/{out_path}/{space}/{dataset}/{today}-LDR_GT_BOX"):
            ...
            json_files = [f for f in os.listdir(f"{base_path}/{working_done_path}/{space}/{dataset}/annotations")
                          if f.endswith('.json')]
            ...
            for idx, json_file in enumerate(json_files):
                ...
                # 후처리 함수
                frame_metadata = BOX.change_frame_metadata(idx+1, pcdfilenum, log_start_time)
